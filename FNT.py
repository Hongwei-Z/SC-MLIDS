...
class Packet(Sensor, Dataset):
    def __init__(self, pid, timestamp, sid, status, location, data, auditee1, auditee2):
...
class FishingNet(object):
    ...
    def new_packet(self, timestamp, sid, status, location, data):  # Create a new packet.
        if self.pid == 0:  # First packet #0.
            packet = Packet(self.pid, timestamp, sid, status, location, data, None, None)
            ...
            if self.pid in self.edge:  # Packets at the initial network edge have only one auditee.
                ...
                packet = Packet(self.pid, timestamp, sid, status, location, data, adt, None)
                ...
                packet = Packet(self.pid, timestamp, sid, status, location, data, adt1, adt2)
